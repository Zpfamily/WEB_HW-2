...
class AddContact:
    ...
    def add_contact(self):
            ...
            if inputs != '':
                ...
                self.assistant.contacts.append(record)

class ConsoleUserView(UserView):
    def __init__(self, assistant, nbook):
        self.assistant = assistant
        self.nbook = nbook
        self.save_data_on_exit = True
    ...
    def interact(self):
        ...
        while True:
            command = prompt("Enter a command: ", completer=completer).lower()
                    
            if command == 'help':
                print(f"Available commands: {', '.join(commands)}")
            elif command == 'add_contact':
                AddContact(self.assistant).add_contact()
            elif command == 'show_contacts':
                self.assistant.show_contacts(self.table_adressbook)
            elif command == "birthday_day":
                self.assistant.get_birthdays(self.table_adressboo0k)
            elif command == 'search_contact':
                self.clear_table()
                name = input('Enter name to search: ')
                self.assistant.search_contact(name, self.table_adressbook)
            elif command == 'edit_contact':
                name = input('Enter contact name to edit: ')
                self.assistant.edit_contact(name, self.table_adressbook)
            elif command == 'delete_contact':
                name = input('Enter contact name to delete: ')
                self.assistant.delete_contact(name, self.table_adressbook)
                ...
                self.nbook.add_note(self.table)
            elif command == 'all_note':
                self.nbook.all_note(self.table)
            elif command == 'sort_note':
                self.nbook.sort_note(self.table)
            elif command == 'find_note':
                self.nbook.find_note(self.table)
            elif command == 'change_note':
                self.nbook.change_note(self.table)
            elif command == 'delete_note':
                self.nbook.delete_note(self.table)
            elif command == 'exit':
                print("Goodbye!")
                self.save_data_on_exit = False 
                raise SystemExit
            else:
...
class PersonalAssistant:
    ...
    def save_data_on_exit(self):
        try:
            ...
            self.console.print(f"[red]Error while saving data: {e}[/]")
            traceback.print_exc()
    ...
    def delete_contact(self, name, table_adressbook):
        ...
        if found_contact:
            ...
            self.console.print(f'[red]Contact {name} not found. Unable to delete.[/]')
...
def main():
    ...
    user_view = ConsoleUserView(assistant, nbook)

    while True:
        user_view.display()  # Display contact data using ConsoleUserView
        user_view.interact()  # User interaction using ConsoleUserView
